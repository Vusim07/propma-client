...
const PropertyForm: React.FC = () => {
  ...
  const { properties, fetchProperties, addProperty, updateProperty, isLoading, error } = useAgentStore();
  
  const [formData, setFormData] = useState<Omit<Property, 'id' | 'created_at' | 'application_link'>>({
    owner_id: user?.id || '',
    address: '',
    city: '',
    state: '',
  ...
  useEffect(() => {
    if (id && properties.length > 0) {
      ...
      if (propertyToEdit) {
        ...
        const formattedDate = new Date(propertyToEdit.available_from).toISOString().split('T')[0];
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) => {
    ...
    if (name === 'rent' || name === 'bedrooms' || name === 'bathrooms' || name === 'square_feet') {
      setFormData({
        ...formData,
        [name]: name === 'bedrooms' || name === 'bathrooms' ? parseInt(value) : parseFloat(value),
      });
    } else {
      setFormData({
        ...formData,
    ...
    <div>
      ...
      <form onSubmit={handleSubmit}>
        ...
        <Card className="mb-6">
          ...
          <CardContent>
            <p className="text-sm text-gray-600 mb-4">
              Add image URLs for this property. These will be displayed to potential tenants.
            </p>
