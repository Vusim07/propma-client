...
import { FileText, CheckCircle, XCircle, Clock, Search, Filter, ArrowRight, Home } from 'lucide-react';
import { showToast } from '../../utils/toast';

const ReviewApplications: React.FC = () => {
  const { user } = useAuthStore();
  const { applications, properties, fetchApplications, fetchProperties, updateApplicationStatus, isLoading } = useAgentStore();
  ...
  }, [user, fetchApplications, fetchProperties]);
  
  const handleStatusChange = async (applicationId: string, status: 'pending' | 'approved' | 'rejected', notes?: string) => {
    try {
  ...
  const getPropertyAddress = (propertyId: string) => {
    ...
    return property ? `${property.address}, ${property.city}, ${property.state} ${property.zip}` : 'Unknown property';
  };
            ...
            <Card key={application.id}>
              <CardContent className="p-6">
                <div className="flex flex-col md:flex-row justify-between mb-4">
                  <div>
                    <h2 className="text-lg font-semibold mb-1">Application #{application.id}</h2>
                  ...
                  <div>
                    ...
                    <div className="bg-gray-50 p-4 rounded-lg">
                      <div className="mb-2">
                        ...
                        <p className="font-medium">#{application.property_id}</p>
                      </div>
                      <div>
                        <p className="text-sm text-gray-500">Address</p>
                        <p className="font-medium">{getPropertyAddress(application.property_id)}</p>
                      </div>
                    </div>
                  </div>
                </div>
                    ...
                    {application.status === 'pending' && (
                      <>
                        <Button
                          variant="primary"
                          size="sm"
                          onClick={() => handleStatusChange(application.id, 'approved', 'Application approved after review.')}
                        >
                          <CheckCircle size={16} className="mr-1" />
                          Approve
                        </Button>
                        <Button
                          variant="danger"
                          size="sm"
                          onClick={() => handleStatusChange(application.id, 'rejected', 'Application rejected after review.')}
                        >
                          <XCircle size={16} className="mr-1" />
                          Reject
                        </Button>
